...
При повторных запросах URL берётся из Redis без SELECT-запроса к БД. Однако счётчики кликов (`clicks`, `unique_clicks`) всё равно обновляются в PostgreSQL через асинхронные UPDATE-запросы (fire-and-forget), поэтому статистика актуальна при любом варианте.
